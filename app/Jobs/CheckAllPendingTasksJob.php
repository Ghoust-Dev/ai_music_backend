...
class CheckAllPendingTasksJob implements ShouldQueue
{
    ...
    protected function shouldUpdateContent(GeneratedContent $content, array $taskData): bool
    {
        ...
        return $content->status !== $newStatus || 
               ...
               ($newStatus === 'processing' && !empty($taskData['audio_url']) && empty($content->streaming_url)) ||
    ...
    protected function updateContentFromTask(GeneratedContent $content, array $taskData): void
    {
        ...
        if ($status === 'processing' && !empty($taskData['audio_url'])) {
            $updates['streaming_url'] = $taskData['audio_url'];
            
            Log::debug('Bulk check: Streaming URL available for processing task', [
                'content_id' => $content->id,
        ...
        // If failed, update error message and clear streaming_url
        if ($status === 'failed') {
            $updates['error_message'] = $taskData['error_message'] ?? 'Generation failed';
            $updates['completed_at'] = now();
            $updates['streaming_url'] = null; // Clear any existing streaming URL
        }
    ...
    protected function shouldRun(): bool
    {
        ...
        // Check minimum frequency (don't run more than once every 2.5 minutes - 50% reduction)
        $lastRun = Cache::get($frequencyKey);
        if ($lastRun && now()->diffInSeconds($lastRun) < 150) { // 2.5 minutes = 150 seconds
            return false;
        }
    ...
    {
        // Schedule next run in 5 minutes (50% reduction)
    ...
    {
        // Schedule retry in 2.5 minutes (50% reduction)
        $delay = now()->addSeconds(150); // 2.5 minutes
